...
def encrypt_message(message, keys):
    ...
    #turn the ciphertext into a string
    ciphertext_string = ''.join(format(i, '08b') for i in ciphertext)
    
    return [ciphertext, ciphertext_string]
...
def decrypt_message(ciphertext, keys):
    ciphertext_binary = ''
...
def main():
    ...
    ciphertext = encrypt_message(message, keys)

    print(f"Ciphertext: {ciphertext[1]}")

    decrypted_message = decrypt_message(ciphertext[0], keys)
